...
/**
 * Internal: load and validate a macros file. Single source of truth for parsing.
 * Returns { ok: true, data } on success, or { ok: false, code, message } on any failure.
 * Missing or empty file → ok: true with defaultData().
 ...
 */
function loadMacrosFile(macrosPath) {
...
function throwFromResult(result) {
...
function readMacrosSync() {
  const result = loadMacrosFile(getMacrosPath());
...
function tryReadMacrosFile(macrosPath) {
  return loadMacrosFile(macrosPath);
}
...
function readMacrosAtPath(macrosPath) {
  const result = loadMacrosFile(macrosPath);
  if (!result.ok) throwFromResult(result);
  return result.data;
}
